...
class KongRun extends FlameGame with TapDetector, HasCollisionDetection {
  ...
  /// This method add the already created [Kong]
  /// and [EnemyManager] to this game.
  void startGamePlay() {
    _kong = Kong(images.fromCache('DinoSprites - tard.png'), playerData);
  ...
  void _disconnectActors() {
    _kong.removeFromParent();
    _enemyManager.removeAllEnemies();
    _enemyManager.removeFromParent();
  }

  void superpower() {
    if (playerData.superpower==1) {
      _enemyManager.removeAllEnemies();
    }
  }
  ...
  void onTapDown(TapDownInfo info) {
    // Make kong jump only when game is playing.
  ...
  Future<PlayerData> _readPlayerData() async {
    final playerDataBox =
        await Hive.openBox<PlayerData>('KongRun.PlayerDataBox');
    final playerData = playerDataBox.get('KongRun.PlayerData');

    // If data is null, this is probably a fresh launch of the game.
    if (playerData == null) {
      // In such cases store default values in hive.
      await playerDataBox.put('KongRun.PlayerData', PlayerData());
    }

    // Now it is safe to return the stored value.
    return playerDataBox.get('KongRun.PlayerData')!;
  }

  /// This method reads [Settings] from the hive box.
  Future<Settings> _readSettings() async {
    final settingsBox = await Hive.openBox<Settings>('KongRun.SettingsBox');
    final settings = settingsBox.get('KongRun.Settings');
    ...
    // Now it is safe to return the stored value.
    return settingsBox.get('KongRun.Settings')!;
  }
